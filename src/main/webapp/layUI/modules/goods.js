...
function getClassfy() {
    var html ="";
    $.ajax({
        url:"/crs/goods/getClassfy.do",       //要处理的页面
        type:"POST",               //提交方式
        dataType:"JSON",          //返回的数据类型，TEXT字符串 JSON返回JSON XML返回XML；dataType中T要大写！！
        success:function(data){          //回调函数，data为形参，是从login-cl.php页面返回的值
            var result = data.classifyList;
            for (var i =0;i<result.length;i++){
                //   <option value="1">layer</option>
                html += '<option value="'+result[i].id+'">'+result[i].c_name+'</option>' ;
            }
           $("#classfy").append(html);
            renderForm();
        }
...
function submitAjax(obj) {
    var a = $("#sellerForm").serialize();
    $("#classfyName").val($("#classfy").find("option:selected").text());
    // debugger;
    $.ajax({
        type: "POST",
        data: $("#sellerForm").serialize(),
        url: "/crs/goods/editGoods.do",
